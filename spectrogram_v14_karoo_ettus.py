...
parser.add_argument('--samp_rate', help='Sample Rate (MHz)', default=25.0)
parser.add_argument('--fmin', help = 'The minimum value (MHz) of the SDR center frequency (f_c). Frequencies to be measured and recorded per spectra are are in range f_c +/ (SAMP_RATE - BW_OVERLAP/2) ', default =30. )
parser.add_argument('--fmax', help = 'The maximum value (MHz) of the SDR center frequency (f_c). Frequencies to be measured and recorded per spectra are are in range f_c +/ (SAMP_RATE - BW_OVERLAP/2) ', default =490. )
parser.add_argument('--overlap',help = 'The amount of bandwidth (MHz) to be ignored during each FFT, to avoid the effects of analog filtes and DSP at band edges. BW_OVERLAP/2 is ignored from each edge of FFT', default =5)
...
parser.add_argument('--ant', help = 'This is the name of the antenna connected to the SDR',default=str('SKALA_MRAO'))
...
PLOT_EACH_SWEEP = True #Should only be used as a diagnostic. 
...
STR_FILE_TEMP = '/Users/josaitis/gnuradio_tutorials/fft_accum_temp/fft_accum_temp.txt'
...
STR_FILE_CAL_NOISE = '/Users/josaitis/gnuradio_tutorials/noise_source_cal_patch_2_files_and_running_mean_1st.csv'#noise_source_cal_patch_2_files_and_running_mean_1st.csv'#noise_source_cal_20kHz_RBW_250_points_prep.csv'#spectrum analysis of noise source, measured by keysight
STR_FILE_CAL_SDR = '/Users/josaitis/gnuradio_tutorials/ettus_30-490_25BW_5overlap_dot25s_gain20.txt'#HackRF_noise_source_gains_0_20_20_BW_20_sizefft_2048.txt'#These are the SDR amplitude (=sqrt power) values when the noise source is connected, measured via spectrogram_v4_working
STR_FILE_SDR_NO_CONNECTION = '/Users/josaitis/gnuradio_tutorials/ettus_terminated_connection.txt'#SDR_nothing_connected_1s.txt'
STR_CALIBRATION = '/Users/josaitis/gnuradio_tutorials/USE_ettus_cal_10MHzspacing_25MHzsamp.csv'#latest_calibration_patched_mean.csv'
...
def calibrate_noise_source(freqs):
	...
	print('Done.')
	np.save('arr_ii_of_length_SDR_ettus.npy',arr_ii_of_length_SDR)
	np.save('noise_source_dbm_ref_ettus.npy',noise_source_dbm_ref)
	np.save('cal_noise_source_linear_ettus.npy',cal_noise_source_linear)
	np.save('SDR_no_connection_ettus.npy',SDR_no_connection)
...
#how would you like to get your one-time-calculated calibration arrays? Calc here, or load. Loading is much faster, calc only if first time or files were deleted.
...
#arr_ii_of_length_SDR=np.load('arr_ii_of_length_SDR_ettus.npy')
#SDR_no_connection=np.load('SDR_no_connection_ettus.npy')
#noise_source_dbm_ref = np.load('noise_source_dbm_ref_ettus.npy')
#cal_noise_source_linear=np.load('cal_noise_source_linear_ettus.npy')
